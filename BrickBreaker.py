...
def main():
    ...
    paddle = Paddle(paddle_x, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, "orange")
    ball = Ball(WIDTH/2, paddle_y - BALL_RADIUS, BALL_RADIUS, "white")

    #gen aleatoria de tijolos
    brick_rows = random.randint(2,5)
    brick_cols = random.randint(5,15)
    rows = brick_rows
    cols = brick_cols
    ...
    while run:
        ...
        if ball.y + ball.radius >= HEIGHT:
            ...
            ball.set_vel(3, ball.VEL * -1)

        if lives <= 0:
            bricks = generate_bricks(rows, cols)
            lives = 3
            reset()
            display_text("Voce perdeu!", 'red')

        if len(bricks) == 0:
            bricks = generate_bricks(rows, cols)
            lives = 3
            reset()
            display_text("Voce ganhou!", 'green')
